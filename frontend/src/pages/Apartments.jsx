      ...
      <div className="container mx-auto px-6">
        <div className="mb-20 pt-10 flex flex-col md:flex-row justify-between items-end gap-8">
          <div className="max-w-2xl">
            ...
            <h1 className="text-5xl md:text-7xl font-serif font-bold text-white leading-tight">All Residences</h1>
          </div>
          <p className="text-gray-200 max-w-md text-lg font-light leading-relaxed">
            Explore our complete collection of premium apartments, designed for ultimate comfort, privacy, and luxury.
          </p>
        </div>

        {loading ? (
          <div className="py-32 text-center text-gray-300 font-sans tracking-widest uppercase">Loading residences...</div>
        ) : (
          <div className="flex flex-col border-t border-dark-800 mt-16">
            {rooms.map((room, idx) => (
              <div key={room.id} className="group relative border-b border-dark-800 overflow-hidden">
                {/* Background Image Reveal */}
                <div className="absolute inset-0 z-0 bg-dark-900 overflow-hidden">
                  {room.image_url && (
                    <img src={`/Images/Room ${(idx % 5) + 1}.jfif`} alt={room.name} className="w-full h-full object-cover opacity-0 group-hover:opacity-30 transition-all duration-1000 scale-110 group-hover:scale-100" />
                  )}
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center p-8 md:p-16 gap-12 transition-transform duration-500 group-hover:translate-x-4">
                  <div className="max-w-3xl">
                    ...
                    <Link to={`/room/${room.id}`}>
                      <h3 className="text-4xl md:text-6xl font-serif font-bold text-white group-hover:text-brand-500 transition-colors duration-500 leading-tight mb-6">{room.name}</h3>
                    </Link>
                ...
                {room.status === 'occupied' && (
                  <div className="absolute top-8 right-8 z-20 bg-amber-500/10 border border-amber-500/50 backdrop-blur-md px-4 py-2 text-amber-500 text-xs font-bold uppercase tracking-widest rounded-full">
                    Currently Booked
                  </div>
                )}
              </div>
