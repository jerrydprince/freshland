...
const StatCard = ({ title, value, icon, trend, delayClass, glowColor = "from-brand-500/25" }) => (
  <div 
    className={`glass-panel p-6 rounded-2xl relative overflow-hidden group hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-brand-500/10 transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both border border-slate-400 hover:border-brand-500/30 ${delayClass}`}
  >
  ...
  return (
    <div className="pb-12">
      <div className="mb-8 flex justify-between items-end animate-in fade-in slide-in-from-left-4 duration-700">
        ...
        <button className="hidden md:flex items-center gap-2 bg-dark-800 border border-slate-400 hover:border-brand-500/50 text-white px-4 py-2 rounded-lg transition-all shadow-sm font-sans font-semibold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {(() => {
          if (hasAccess('Reservations') || hasAccess('Front Desk')) {
            return (
              <div className="lg:col-span-2 glass-panel p-6 rounded-2xl animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500 fill-mode-both border border-slate-400">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="text-gray-200 text-xs uppercase tracking-wider border-b border-slate-400/50">
                        <th className="pb-3 font-semibold px-2 border-r border-slate-400/50">Guest</th>
                        <th className="pb-3 font-semibold px-2 border-r border-slate-400/50">Room</th>
                        <th className="pb-3 font-semibold px-2 border-r border-slate-400/50">Date</th>
                        <th className="pb-3 font-semibold px-2 border-r border-slate-400/50">Status</th>
                      ...
                      {loading ? (
                        ...
                        (recentBookings || []).map((item, i) => {
                          ...
                          return (
                            <tr 
                              key={item.id || i} 
                              className="border-b border-slate-400/30 hover:bg-dark-700/20 transition-colors group animate-in fade-in slide-in-from-left-4 duration-500 fill-mode-both"
                    ...
                    <tbody className="text-sm text-gray-300">
                      {pendingTasks.length === 0 ? (
                        ...
                        pendingTasks.map((task) => (
                          <tr key={task.id} className="border-b border-slate-400/30 hover:bg-dark-700/20 transition-colors">
                            <td className="py-4 px-2">
                    ...
                    <tbody className="text-sm text-gray-300">
                      {activeTickets.length === 0 ? (
                        ...
                        activeTickets.map((ticket) => (
                          <tr key={ticket.id} className="border-b border-slate-400/30 hover:bg-dark-700/20 transition-colors">
                            <td className="py-4 px-2">
          ...
          return (
            <div className="lg:col-span-2 glass-panel p-8 rounded-2xl flex flex-col justify-center items-center text-center animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500 fill-mode-both border border-slate-400">
              ...
              <div className="bg-dark-900 border border-slate-400/60 p-4 rounded-xl max-w-sm w-full mb-6 font-semibold flex justify-between items-center text-xs">
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col animate-in fade-in slide-in-from-bottom-8 duration-700 delay-700 fill-mode-both border border-slate-400">
          <div className="flex justify-between items-center mb-6">
            ...
            {pulseItems.length === 0 ? (
              ...
              pulseItems.map((item, idx) => (
                <div className="group" key={idx}>
                  ...
                  <div className="w-full bg-dark-900/50 h-2.5 rounded-full overflow-hidden border border-slate-400/50">
                    <div 
          ...
          <div className="mt-8 pt-6 border-t border-slate-400/50">
